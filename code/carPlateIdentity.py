...
def pre_process(orig_img):
    ...
    cv2.imwrite('../images/opencv_output/gray_img.png', gray_img)

    blur_img = cv2.blur(gray_img, (3, 3))
    #cv2.imshow('blur', blur_img)
    cv2.imwrite('../images/opencv_output/blur.png', blur_img)

    sobel_img = cv2.Sobel(blur_img, cv2.CV_16S, 1, 0, ksize=3)
    sobel_img = cv2.convertScaleAbs(sobel_img)
    #cv2.imshow('sobel', sobel_img)
    cv2.imwrite('../images/opencv_output/sobel.png', sobel_img)
    ...
    cv2.imwrite('../images/opencv_output/mix.png', mix_img)

    mix_img = mix_img.astype(np.uint8)

    ret, binary_img = cv2.threshold(mix_img, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    #cv2.imshow('binary',binary_img)
    cv2.imwrite('../images/opencv_output/binary.png', binary_img)

    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(21,5))
    close_img = cv2.morphologyEx(binary_img, cv2.MORPH_CLOSE, kernel)
    #cv2.imshow('close', close_img)
    cv2.imwrite('../images/opencv_output/close.png', close_img)
...
def verify_color(rotate_rect,src_image):
    ...
    cv2.imwrite('../images/opencv_output/floodfill.png', flood_img)
    cv2.imwrite('../images/opencv_output/flood_mask.png', mask)
    #======================调试用======================#
...
def locate_carPlate(orig_img,pred_image):
    ...
    for i,contour in enumerate(contours):
        ...
        if verify_scale(rotate_rect):
            ...
            cv2.imwrite('../images/opencv_output/opencv_.png', car_plate)
            #========================调试看效果========================#
            carPlate_list.append(car_plate)

    cv2.imshow('contour', temp1_orig_img)
    cv2.imwrite('../images/opencv_output/contour.png', temp1_orig_img)
...
def get_chars(car_plate):
    ...
    cv2.imwrite('../images/opencv_output/car.jpg',car_plate)
    cv2.imwrite('../images/opencv_output/plate.jpg', plates)
    ...
    for i,addr in enumerate(char_addr_list):
        ...
        cv2.imwrite('../images/opencv_output/{}.jpg'.format(i), char_img)
    return char_imgs
...
if __name__ == '__main__':
    ...
    char_model_path = os.path.join(cur_dir,'../carIdentityData/model/char_recongnize/model.ckpt-570.meta')
    ...
    cv2.imwrite('../images/opencv_output/cnn_plate.png', car_plate)
